...
Write-Host "🔑 Azure Subscription '$AzureSubscriptionID' selected." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ ERROR: CustomerSite build failed" -ForegroundColor Red
    exit 1
}
Write-Host "✅ CustomerSite built successfully" -ForegroundColor Green
Write-Host ""
...
if (Test-Path "./Publish/CustomerSite/web.config") {
    Write-Host "✅ web.config found in publish output (auto-generated by dotnet publish)" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host "⚠️  WARNING: web.config not found in publish output" -ForegroundColor Yellow
    Write-Host ""
}
...
# Remove-Item -Path ./Publish -Recurse -Force
Write-Host "✅ Deployment complete (Publish directory kept for debugging)" -ForegroundColor Green
Write-Host ""
...
Write-Host "✅ Customer Portal Deployment Completed!" -ForegroundColor Green
Write-Host "==========================================" -ForegroundColor Green
Write-Host ""
